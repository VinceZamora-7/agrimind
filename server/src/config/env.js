...
function createConfig(serverDir, mode) {
  ...
  return {
    ...
    camera: {
      ...
      warmupFrames: number("CAMERA_WARMUP_FRAMES", 30),
      captureSkipFrames: number("CAMERA_CAPTURE_SKIP_FRAMES", 5),
      exposureRetuneMs: number("CAMERA_EXPOSURE_RETUNE_SECONDS", 300) * 1000,
      lowLightThreshold: number("CAMERA_LOW_LIGHT_THRESHOLD", 45),
      lowLightExposure: number("CAMERA_LOW_LIGHT_EXPOSURE", 5000),
      lowLightGain: number("CAMERA_LOW_LIGHT_GAIN", 100),
    ...
    pir: {
      ...
      rearmLowMs: number("PIR_REARM_LOW_SECONDS", 2) * 1000,
      cooldownMs: number("DETECTION_COOLDOWN_SECONDS", 5) * 1000,
      groupingMs: number("EVENT_GROUPING_SECONDS", 5) * 1000,
    },
    duplicate: {
    ...
    gemini: {
      ...
      minIntervalMs: number("GEMINI_MIN_INTERVAL_SECONDS", 5) * 1000,
    ...
    semaphore: {
      ...
      categoryCooldownMs: {
        ...
        PLANT_ABNORMAL: number("PLANT_SMS_COOLDOWN_SECONDS", 60) * 1000,
    ...
    pairing: {
      ...
      allowAlways: bool("PAIRING_ALLOW_ALWAYS", true),
    ...
    environmentSensor: {
      enabled: bool("SHT41_ENABLED", mode.startsWith("orange_pi")),
      bus: number("SHT41_I2C_BUS", 0),
      address: number("SHT41_I2C_ADDRESS", 0x44),
      toolPath: process.env.I2C_TRANSFER_PATH || "/usr/sbin/i2ctransfer",
      sampleMs: number("SHT41_SAMPLE_SECONDS", 5) * 1000,
      staleMs: number("SHT41_STALE_SECONDS", 180) * 1000,
      windowSize: Math.max(1, Math.min(20, number("SHT41_AVERAGING_SAMPLES", 5))),
      measurementDelayMs: number("SHT41_MEASUREMENT_DELAY_MS", 20),
      commandTimeoutMs: number("SHT41_COMMAND_TIMEOUT_MS", 2000),
    },
    weather: {
    ...
    weatherSimulator: {
      enabled: bool("WEATHER_SIMULATOR_ENABLED", false),
      url: process.env.WEATHER_SIMULATOR_URL || "",
      pollMs: Math.max(1000, number("WEATHER_SIMULATOR_POLL_SECONDS", 3) * 1000),
      timeoutMs: number("WEATHER_SIMULATOR_TIMEOUT_SECONDS", 5) * 1000,
      storePath: path.join(projectDir, "storage", "logs", "weather-simulator.json"),
    },
    pagasa: {
      enabled: bool("PAGASA_ALERTS_ENABLED", true),
      feedUrl: process.env.PAGASA_ALERT_FEED_URL || "https://publicalert.pagasa.dost.gov.ph/feeds/",
      refreshMs: number("PAGASA_ALERT_REFRESH_MINUTES", 5) * 60 * 1000,
      timeoutMs: number("PAGASA_ALERT_TIMEOUT_SECONDS", 20) * 1000,
      maxEntries: Math.max(1, Math.min(100, number("PAGASA_ALERT_MAX_ENTRIES", 50))),
      storePath: path.join(projectDir, "storage", "logs", "pagasa-alerts.json"),
    },
  };
}
